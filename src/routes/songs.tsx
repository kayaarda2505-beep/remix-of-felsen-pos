...
export const Route = createFileRoute("/songs")({
  head: () => ({ meta: [{ title: "Song-Wünsche — SAINTS POS" }] }),
  component: SongsPage,
});
...
function SongsPage() {
  ...
  useEffect(() => {
    const ch = supabase
      .channel(`song_requests_${Math.random().toString(36).slice(2)}`)
      .on(
        "postgres_changes",
  ...
  const playSong = async (s: SongRequest) => {
    ...
    try {
      ...
      if (!uri) {
        // Fallback: per Suche den besten Treffer finden
  ...
  return (
    <SettingsPage
      title="Song-Wünsche"
      subtitle="Wünsche von Gästen über den Tisch-QR-Code — live"
    >
                ...
                {s.status === "new" ? (
                  <>
                    ...
                    <button
                      ...
                      className="rounded-xl bg-success/15 hover:bg-success/25 text-success w-9 h-9 flex items-center justify-center"
